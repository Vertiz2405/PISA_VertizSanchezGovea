...
def cargar_datos_parquet(parquet_folder, parquet_files, target_date):
    """Carga y filtra datos de un archivo Parquet en una ventana de 24 horas."""
    ...
    end_time = target_datetime + pd.Timedelta(hours=18)
    ...
    if df_result:
        ...
        """output_csv = f"data/data_{target_date}.csv"
        os.makedirs("data", exist_ok=True)
        df_final.to_csv(output_csv, index=False)"""
        print(f"Datos guardados con {df_final.shape[0]} filas y {df_final.shape[1]} columnas.")
        return df_final
    else:
